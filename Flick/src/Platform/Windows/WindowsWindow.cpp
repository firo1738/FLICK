...
namespace Flick
{
	...
	{
		FI_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		FI_CORE_INFO("Creating a {0} window ({1}, {2})", props.Title, props.Width, props.Height);

		if (s_GLFWWindowCount == 0)
		{
			FI_PROFILE_SCOPE("glfwInit");
		...
		{
			FI_PROFILE_SCOPE("glfwCreateWindow");
		#if defined(FI_DEBUG)
